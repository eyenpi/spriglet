...
Successful [owner-approved PR builds](../CI/README.md) provide Actions artifacts (a GitHub sign-in is needed to download them), retained for seven days. Branch pushes and tags do not run CI. Publish release packages explicitly with the [local release publisher](../ReleaseNotes/README.md) from the exact clean tag revision. Public GitHub Release downloads do not require Xcode or a GitHub account, and release assets do not expire with Actions artifacts.

Publication verifies the package hashes, version, build, clean source revision, architecture and signature status against the validated tag on `main`. Retries accept only identical published assets; they never replace a release. Unsigned packages are restricted to preview entries. Stable release packaging is deliberately blocked until Developer ID signing/notarization is configured; the workflow cannot silently ship an unsigned stable build. PR validation uses no signing secrets or release-write permission.
