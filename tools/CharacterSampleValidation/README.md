...
python3 tools/CharacterSampleValidation/validate_assets.py \
  ...
  --output docs/results/character-sample/assets.json
```
...
tools/CharacterSampleValidation/.build/native/NativeSampleCheck \
  ...
  --asset-report docs/results/character-sample/assets.json \
  --output docs/results/character-sample/native-lifecycle.json
```
...
open -n "$PWD/tools/CharacterSampleValidation/.build/CharacterSampleValidation.app" --args \
  ...
  --asset-report "$PWD/docs/results/character-sample/assets.json" \
  --output "$PWD/docs/results/character-sample/native-review.json" \
...
The retained [test summary](../../docs/results/character-sample/tests.json) records actual completed runs. [Contact preflight](../../docs/results/character-sample/contact-preflight.json) measures the authored metadata before runtime export and explicitly excludes final PNG/manifest agreement. The final asset and native reports above must be produced from the finished resource set; this README does not claim an unperformed run.
