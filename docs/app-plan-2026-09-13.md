...
Research checked on **13 September 2026**. This is the initial plan, retained from before implementation. See the [public preview](public-preview.md) for the current app and verification status.

Source brief: private local note (not published). The user confirmed **a local companion first**. Visual style remains a recommendation, not a final character-design decision.
...
[View the illustrative style comparison](concepts/style-comparison.png). It is a generated concept study, not a production model, rig, sprite sheet, or proof of animation quality. Its character design is provisional. [Generation prompt and provenance](concepts/style-comparison-prompt.md).
