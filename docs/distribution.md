...
Research/tool checks used Xcode 26.6 (17F113), Swift 6.3.3, and the installed `codesign`, `notarytool`, `stapler`, `lipo`, `vtool`, and `syspolicy_check` interfaces. The local preflight and focused verifier tests pass; the missing-identity distribution path stops as intended. A read-only check of the pre-polish Release app passed metadata and all 244 PNGs but correctly rejected its injected `get-task-allow=true`; the packaging path disables base entitlement injection and signs from the sandbox-only entitlement file. The integrated Release archive and extracted local ZIP now pass the [retained package checks](results/public-preview/local-package.json), including all 244 PNGs and sandbox-only entitlements. That artifact is ad hoc signed, explicitly labelled LOCAL-UNSIGNED, and is not uploaded as a normal app download. Developer ID signing, Apple acceptance, stapling, and fresh-machine launch remain **unverified until signing is configured and the full path succeeds**.
