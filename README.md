...
Run the app tests with `./scripts/test.sh`. Build without launching with `./scripts/build.sh Debug` or `./scripts/build.sh Release`. The GitHub Actions workflow also runs the Python validators and checks every animation frame against the authored foot-contact data.
