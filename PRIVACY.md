...
When Acorn's top-bar eyes are visible, Spriglet reads the current pointer location to aim them. The position stays in memory only for the current animation; Spriglet does not save a pointer path or send it anywhere.
