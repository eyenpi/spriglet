...
Local preferences remain until you change them or remove the app's local preference storage. Clear Recent Preferences in Settings > Companion removes recent interaction values and their recovery copy immediately. Removing the app alone may leave macOS's sandbox preferences behind. For help removing remaining local settings, use Get Support in the app. There is no account or server-side data for us to delete, and we cannot access or remotely remove your local preferences. macOS manages the separate Launch at Login choice; turn it off in Settings > General or System Settings > General > Login Items & Extensions.
...
Privacy Policy is available from the leaf menu and Settings > General, including a bundled copy that works offline. Read Online and Get Support open your default browser only when selected. GitHub separately handles visits to the source repository and any information you choose to submit in an issue. Do not include private information in a public issue. Contact the developer through https://github.com/eyenpi/spriglet/issues for support or privacy questions.
